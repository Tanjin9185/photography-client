...
const Services = ({ service }) => {
    // const { title, price, subtitle, content } = props?.service;
    console.log(service)
    return (
                ...
                <div className="grid grid-cols-1 gap-16 lg:grid-cols-3 sm:gap-8">
                    <div className="border-0 rounded-none shadow-none card sm:rounded-lg md:border">
                        <div className="flex flex-col justify-between p-6 border-b border-gray-200">
                            <p className="mb-1 text-lg font-semibold text-yellow-600">{service?.title}</p>
                            <p className="pb-0 my-2 font-mono text-4xl font-extrabold text-gray-900">${service?.price}</p>
